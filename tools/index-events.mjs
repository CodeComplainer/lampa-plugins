...
// «interaction/player/video.js ×3» — путь без src/ остаётся однозначным,
// а голого имени файла не хватает: settings.js и params.js в Lampa не по одному.
//
// Номеров строк здесь намеренно нет. Они устаревали при каждом обновлении
// апстрима и давали дифф на полсотни строк, в котором не менялось ничего:
// ни канала, ни payload, ни файла. Теперь любой дифф этого файла означает, что
// событийная поверхность Lampa действительно изменилась. До самого вызова
// доходят грепом по имени канала — файл известен, и это одна команда.
function places(sites) {
    const byFile = new Map()

    for (const site of sites) {
        const file = site.replace(/^src\//, '')

        byFile.set(file, (byFile.get(file) || 0) + 1)
    }

    const parts = [...byFile.entries()].map(([file, count]) => (count > 1 ? `${file} ×${count}` : file))
...
стоит литералом внутри \`.send(…)\` во всех ${sends} местах отправки без исключения.
